...
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Random Binaries NFT Unit Tests:", function () {
          let randomBinariesNFT, deployer, coordinatorMock;

          beforeEach(async function () {
              accounts = await ethers.getSigners();
              deployer = accounts[0];
              await deployments.fixture(["mocks", "random-binaries-nft"]);
              randomBinariesNFT = await ethers.getContract("RandomBinariesNFT");
          ...
          describe("Constructor:", function () {
              it("Sets starting values correctly.", async function () {
                  const tokenURIZero = await randomBinariesNFT.getTokenURIs(0);
          ...
          describe("Function <requestNFT>:", function () {
              it("Fails if the payment is not sent with the request.", async function () {
                  await expect(randomBinariesNFT.requestNFT()).to.be.revertedWith(
                      "RandomBinariesNFT__NotEnoughETHSent"
                  );
              });
              ...
              it("Emits an event and starts a random word request.", async function () {
                  ...
                  await expect(
                      randomBinariesNFT.requestNFT({ value: mintFee.toString() })
                  ).to.emit(randomBinariesNFT, "NFTRequested");
              });
          });
              ...
              it("Mints an NFT after a random number is returned.", async function () {
                  await new Promise(async (resolve, reject) => {
                      randomBinariesNFT.once("NFTMinted", async function () {
                          try {
                              const tokenURI = await randomBinariesNFT.tokenURI("0");
                              const tokenCounter =
                                  await randomBinariesNFT.getTokenCounter();
                              assert.equal(
                      ...
                      try {
                          const mintFee = await randomBinariesNFT.getMintFee();
                          const requestNFTResponse = await randomBinariesNFT.requestNFT(
                              {
